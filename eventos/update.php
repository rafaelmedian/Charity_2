<?php
	...
	if(!empty($_POST)) {
		...
		if(empty($event_type_code)){
			$event_type_code_error = 'Por favor, inserte el tipo de evento';
			$valid = false;
		}

		if(empty($event_name)){
			$event_name_error = 'Por favor, inserte el nombre del evento';
			$valid = false;
		}
		...
		if(empty($event_entrance_fee)) {
			...
			$precioError = 'Inserte solo numeros';
			$valid = false;
		}*/
		...
		if($valid) {
			...
			$sql = "UPDATE INTO events SET event_id = ?, event_type_code = ?, event_name = ?, event_place = ?, event_date = ?, event_entrance_fee = ?";
			$q = $pdo->prepare($sql);
			$q->execute(array(
				$event_id,
				$event_type_code,
			...
			$matricula = $data['event_id'];
			$marca = $data['event_type_code'];
			$modelo = $data['event_name'];
			$ano = $data['event_type_code'];
			$lol = $data['event_place'];
			$asdsa = $data['event_entrance_fee'];
